...
AccessError:
    # FIXME: ??????????????????????????????auth_user??owner????????????????????auth_user??owner????????
...
AccessError:
    # FIXME: ??????????????????????????????auth_user??owner??remove??????????????auth_user??owner??remove????
    - Message with message_id was sent by the authorised user making this request
    - The authorised user is an owner of this channel (if it was sent to a channel) or the **Dreams**
...
# FIXME: ????????message_id????u_id,????????messages[]????????channel??messages??????0??????message_id??????????
# TODO: ??create_session_id????????????channel??????message_id??????
def get_u_id_by_message_id(message_id):
    return get_message_by_message_id(message_id).u_id


# TODO: ????message_id????????message
